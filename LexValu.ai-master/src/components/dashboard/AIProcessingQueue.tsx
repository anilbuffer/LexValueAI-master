...
export async function AIProcessingQueue() {
  ...
  // In CasesTable, a scan is active ("Scanning...") if flags is null and status is not READY/Closed/FAILED
  const activeCases = await prisma.case.findMany({
    where: {
      ...userCaseFilter,
      flags: null,
      status: { notIn: ['READY', 'Closed', 'FAILED'] }
    },
  ...
  // In CasesTable, a scan is completed if status is 'READY' or 'Closed'
  ...
  const completedCases = await prisma.case.findMany({
    ...
    include: {
      documents: {
        select: { summary: true, aiAnalysis: true }
      }
    }
  });
      ...
      <div className="divide-y divide-slate-100 flex-1 overflow-y-auto custom-scrollbar">
        ...
        {completedCases.map((c) => {
          ...
          if (!summaryText) {
            ...
            summaryText = summaryText.split('\n').filter(p => p.trim().length > 0)[0] || summaryText;

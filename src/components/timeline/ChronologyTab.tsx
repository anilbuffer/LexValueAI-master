  ...
  return (
    <div className="flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="bg-slate-50 p-5 border-b border-slate-200">
        <h2 className="text-2xl font-bold text-slate-800">Medical Chronology</h2>
        <p className="text-sm text-slate-500 mt-1">Timeline of {events.length} extracted medical events.</p>
      </div>
        ...
        {events.length === 0 ? (
          ...
          <div className="flex flex-col">
            {events.map((event: any, index: number) => {
              ...
              return (
                <div key={event.id} className="flex flex-col md:flex-row gap-2 md:gap-6 relative group mb-4 md:mb-0">
                  <div className="w-full md:w-[70px] shrink-0 text-left md:text-right flex flex-row md:flex-col items-baseline md:items-end justify-start gap-2 md:gap-0 pt-1 md:pt-1 px-1 md:px-0">
                    <div className="text-[15px] font-bold text-slate-800 leading-none">{month} {day}</div>
                    <div className="text-[11px] font-bold text-slate-400 md:mt-1 tracking-widest uppercase">{year}</div>
                    {details?.time && (
                      <div className="text-[10px] font-bold text-teal-600 md:mt-1.5 tracking-wide uppercase whitespace-nowrap">{details.time}</div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 pb-0 md:pb-2">
                    <div className="bg-white border border-slate-100 rounded-2xl p-4 md:p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] hover:border-teal-200 transition-all duration-300 group-hover:-translate-y-1 mb-2 md:mb-6 break-words">

                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-2 mb-1.5">
                        <div className="flex items-start md:items-center flex-wrap gap-3 flex-1">
                          <h4 className="text-[17px] md:text-[18px] font-semibold text-slate-800">{event.title}</h4>
                          <span className={`text-[11px] px-2 py-0.5 rounded-full border font-bold whitespace-nowrap shrink-0 mt-0.5 md:mt-0 ${getConfidenceStyle(confidence)}`}>
                            {confidence} Confidence
                          </span>
                        </div>
                        {provider && (
                          <div className="flex items-start md:items-center gap-1.5 text-slate-500 text-[14px] md:text-[15px] font-medium md:max-w-[45%] md:justify-end shrink-0">
                            <Stethoscope className="w-4 h-4 shrink-0 mt-[2px] md:mt-0" />
                      ...
                      <p className="text-[14.5px] md:text-[15px] text-slate-600 leading-relaxed mb-4">{descriptionText}</p>
                            ...
                            {complaints && (
                              <div>
                                <div className="flex items-center gap-1.5 mb-2">
                                  ...
                                  <h4 className="text-[11px] font-bold text-slate-400 tracking-wider uppercase">Complaint / Symptoms</h4>
                                </div>
                                <p className="text-[14px] text-slate-700 leading-relaxed">{complaints}</p>
                              </div>
                            )}
                            {diagnosis && (
                              <div>
                                <div className="flex items-center gap-1.5 mb-2">
                                  ...
                                  <h4 className="text-[11px] font-bold text-slate-400 tracking-wider uppercase">Diagnosis</h4>
                                </div>
                                <p className="text-[14px] text-slate-700 leading-relaxed">{diagnosis}</p>
                              </div>
                            )}
                            {treatment && (
                              <div>
                                <div className="flex items-center gap-1.5 mb-2">
                                  ...
                                  <h4 className="text-[11px] font-bold text-slate-400 tracking-wider uppercase">Treatment Rendered</h4>
                                </div>
                                <p className="text-[14px] text-slate-700 leading-relaxed">{treatment}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="mt-5 pt-3 border-t border-slate-50 flex items-center gap-3 w-full min-w-0">
                        <div
                          ...
                          className="flex items-center gap-2 text-[12px] text-teal-700 font-semibold bg-teal-50 border border-teal-100 px-3 py-1.5 rounded-md max-w-full overflow-hidden cursor-pointer hover:bg-teal-100 transition-colors"
                        >

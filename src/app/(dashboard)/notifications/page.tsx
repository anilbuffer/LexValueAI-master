...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    setLocalNotifications(getMockNotifications())
    setIsLoading(false)
  }, [])
  ...
  const filteredNotifications = useMemo(() => {
    return localNotifications.filter(n => {
      if (debouncedSearch) {
        if (!n.message.toLowerCase().includes(debouncedSearch.toLowerCase())) {
          return false;
        }
      }
      ...
      if (dateFilter.from && new Date(n.createdAt) < new Date(dateFilter.from)) return false;
      if (dateFilter.to && new Date(n.createdAt) > new Date(dateFilter.to + 'T23:59:59')) return false;

      return true;
    }).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [localNotifications, debouncedSearch, readFilter, dateFilter])

  const totalLogs = filteredNotifications.length;
  const totalPages = Math.max(1, Math.ceil(totalLogs / ITEMS_PER_PAGE));
  const currentNotifications = filteredNotifications.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);

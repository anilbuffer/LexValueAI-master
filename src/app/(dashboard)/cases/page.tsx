...
import { getMockCases, getMockUsers } from "@/lib/mock-data"
...
export default function CasesPage() {
  ...
  useEffect(() => {
    if (role === 'MANAGING_PARTNER' || role === 'ATTORNEY') {
      ...
      setSubordinates(mockUsers.filter(u => u.role === 'ATTORNEY' || u.role === 'PARALEGAL'))
    }
  }, [role])
  ...
  const filteredCases = useMemo(() => {
    return localCases.filter(c => {
      ...
      if (statusFilter.pending) activeStatusFilters.push('PENDING')
      if (statusFilter.approved) activeStatusFilters.push('APPROVED')
      if (statusFilter.rejected) activeStatusFilters.push('REJECTED')
      if (statusFilter.closed) activeStatusFilters.push('CLOSED')

      if (activeStatusFilters.length > 0) {
        if (!activeStatusFilters.includes(c.approvalStatus) && !activeStatusFilters.includes(c.status)) {
          return false
        }
      }
      ...
      if (dateFilter.from && new Date(c.dateOfInjury) < new Date(dateFilter.from)) return false
      if (dateFilter.to && new Date(c.dateOfInjury) > new Date(dateFilter.to + 'T23:59:59')) return false

      return true
    })
  }, [localCases, debouncedSearch, statusFilter, categoryFilter, dateFilter])
    ...
    setTimeout(() => {
      setLocalCases(prev => prev.map(c => {
        if (selectedCaseIds.includes(c.id)) {
          const attorney = subordinates.find(u => u.id === assignAttorneyId);
          const paralegal = subordinates.find(u => u.id === assignParalegalId);
          return { ...c, assignedUsers: [attorney, paralegal].filter(Boolean) }
        }
        return c;
      }))
  ...
  const submitApproveCase = async () => {
    ...
    setIsApproving(true)
    setTimeout(() => {
      setLocalCases(prev => prev.map(c => c.id === approveCaseTarget.id ? { ...c, approvalStatus: 'APPROVED' } : c))
  ...
  const submitRejectCase = async () => {
    ...
    setIsRejecting(true)
    setTimeout(() => {
      setLocalCases(prev => prev.map(c => c.id === rejectCaseTarget.id ? { ...c, approvalStatus: 'REJECTED', rejectionReason: rejectReason } : c))
  ...
  const submitResubmitCase = async () => {
    ...
    setTimeout(() => {
      setLocalCases(prev => prev.map(c => c.id === resubmitCaseTarget.id ? { ...c, approvalStatus: 'PENDING', rejectionReason: null } : c))
  ...
  const handleCloseCase = async () => {
    ...
    setIsClosing(true);
    setTimeout(() => {
      setLocalCases(prev => prev.map(c => c.id === caseToClose.id ? { ...c, status: 'CLOSED' } : c))
  ...
  const confirmDeleteCase = async () => {
    ...
    setTimeout(() => {
      setLocalCases(prev => prev.filter(c => c.id !== caseToDelete.id))
    ...
    <div className="w-full p-2.5 pb-[15px] font-sans relative flex flex-col gap-[15px]">
      ...
      <div className="bg-white rounded-2xl shadow-md shadow-slate-200/50 border border-slate-200/50 p-5 flex flex-col gap-[20px]">
        ...
        <CasesTable
          ...
          onResubmitCase={(c) => setResubmitCaseTarget(c)}
